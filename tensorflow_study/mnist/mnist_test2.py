...
def train_val_split(x_data, y_data):
    ...
    print(type(x_data))  # <class 'pandas.core.frame.DataFrame'>
    print(x_data.shape)
    print(sys.getsizeof(x_data) / (1024 * 1024))

    # 由于数据值范围在0-255，部分值差异太大，故进行0-1标准化
    x_train = x_data.iloc[0:trainDataSize,].div(255.0)
    print(type(x_train))
    print(x_train.shape)
    ...
    y_train = y_data[:trainDataSize, ].astype(np.float32)  # 需要保证数据类型一致性
    x_val = x_data.iloc[trainDataSize:, ].div(255.0)  # 由于数据值范围在0-255，部分值差异太大，故进行0-1标准化，此为验证Images数据，用来验证后面的模型的准确率
    y_val = y_data[trainDataSize:, ].astype(np.float32)  # 此为Label数据，用来验证后面模型的准确率
...
class CNN():
    ...
    def train_cnn(self, x_train, y_train, x_val, y_val, n):
        ...
        for i in range(100):
            ...
            print("第" + str(i + 1) + "轮，准确率为：" + str(accuracy_n) + "，共花了%ds" % (batchEndTime-batchStartTime))
            if accuracy_n > 0.989:
                break;
    ...
    def load_cnn_model(self, data, filename):  # 此处只加载模型
        try:
            ...
            with tf.Session() as sess:  # 初始化Session
                ...
                pd.DataFrame({'ImageId': np.arange(len(result)) + 1, 'Label': result}).to_csv(fileDir + 'cnn_mnist_submission_20181224.csv',
                                                                                              index=False)  # 输出到CSV文件
