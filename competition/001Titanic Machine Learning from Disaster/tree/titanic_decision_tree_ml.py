...
def resultsAndSaveHandle(results,test_pass_id):
    ...
    submission.to_csv("titanic_simple_randomforest_submission_02.csv", index=False)

def titanic_simple_handle():
    train_X,train_Y,test_X,test_pass_id= loadData()
    print(test_X.shape)

    rfc_clf = RandomForestClassifier(random_state=1,n_estimators=10,min_samples_split=2,min_samples_leaf=1)

    # results = cross_val_score(rfc_clf,train_X,train_Y,cv=5)
    # print(results)
    # print(results.mean())

    rfc_clf.fit(train_X,train_Y)
    results = rfc_clf.predict(test_X)
